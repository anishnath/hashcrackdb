...
public class TigerConsumer implements Runnable {

	protected BlockingQueue queue = null;
	protected Database db;

	public TigerConsumer(BlockingQueue queue, Database md5database) {
		this.queue = queue;
		this.db = md5database;
	}
	...
	public void run() {
		...
		try {
			MessageDigest crypt = MessageDigest.getInstance("tiger");
			while (true) {
				...
				//System.out.println("Tiger Consumer -- " + hexValue.toUpperCase());
